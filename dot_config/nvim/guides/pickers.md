...
`<C-o>` exists because `ClaudeCodeSend` @-mentions a _file range_ and never
...
## Guides: by title or by content

`<leader>?` lists the guides by their titles, which is the right list for "open
the git one" and the wrong one for "which guide mentions `zx`" — eleven titles
hold about eleven words. `<C-g>` switches that picker to a live grep across the
guides directory, and `<C-g>` again switches back. Whatever you typed comes with
you, so a title search that found nothing becomes a content search without
retyping it.

Confirming a content hit opens the guide float on the matching line, centred,
rather than at the top.

Everywhere else `<C-g>` is snacks' `toggle_live`. In the title picker that only
ever warns — a static list cannot go live — and in the content picker grep
already starts live, so what the swap costs is the ability to turn live _off_.
