...
appear for `/` and `?` — those are incremental, and a popup over the buffer would
hide the match you're watching. Ghost text still shows there.
...
| `<C-a>` | insert *every* match at once | vim |
...
You rarely want to *type* the thing you're operating on.
...
| `<C-r>0` | the last *yank* specifically, unaffected by deletes |
...
— delete everything that *doesn't* match.
